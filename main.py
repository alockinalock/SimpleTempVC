...
@bot.tree.command(name="echo-config", description="DEV ONLY")
async def dev_echo_config(interaction: discord.Interaction):
        if bot.working_category is not None:
                await interaction.response.send_message(bot.working_category)
        else:
...
async def command_create(interaction: discord.Interaction, name: str = "Temp", count: int = None):
        ...
        if cog is not None:
                await cog.create_voice(interaction, bot.working_category, name, count)
